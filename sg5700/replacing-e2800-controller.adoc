...
更换控制器后，无法访问设备存储节点。如果E2800系列控制器运行正常、您可以 link:../maintain/placing-appliance-into-maintenance-mode.html["将 E5700SG 控制器置于维护模式"]。
...
IMPORTANT: *可能无法访问数据。&&如果设备中的所有驱动器都启用了安全保护，则新控制器将无法访问设备，直到您使用SANtricity 系统管理器中的“企业管理”窗口解锁受保护的驱动器。
...
.步骤
. 如果 StorageGRID 设备正在 StorageGRID 系统中运行， link:../maintain/placing-appliance-into-maintenance-mode.html["将 E5700SG 控制器置于维护模式"]。
