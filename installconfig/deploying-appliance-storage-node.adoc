...
+
** 要将设备存储节点添加到现有StorageGRID 系统、请参见的说明 link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["正在添加网格节点"]。
** 要在恢复操作中部署设备存储节点、请参见说明 link:../maintain/recovering-storagegrid-appliance-storage-node.html["恢复设备存储节点"]。
...
NOTE: 如果要将存储节点设备部署为节点克隆目标、请在此停止部署过程并继续link:../commonhardware/appliance-node-cloning-procedure.html["节点克隆操作步骤"]。
...
NOTE: 如果您需要一次部署多个设备存储节点、则可以使用自动执行安装过程 `configure-sga.py` 设备安装脚本。
