...
如果您需要部署任何StorageGRID 设备存储节点、请参见 link:../installconfig/deploying-appliance-storage-node.html["部署设备存储节点"]。

您也可以在打开节点电源之前重新映射节点端口或增加节点的 CPU 或内存设置。

.开始之前
* 您已查看如何操作 link:planning-and-preparation.html["规划并准备安装"]，您了解软件， CPU 和 RAM 以及存储和性能的要求。
...
IMPORTANT: 在生产部署中、不要在一个虚拟机服务器上运行多个存储节点。对每个存储节点使用专用虚拟机主机可提供一个隔离的故障域。
